...
# Usage:
# Save this file to $HOME/.cisco/csd-wrapper.sh
# export CSD_HOSTNAME=yourvpnhost
# sudo -E openconnect \
#    --csd-user=$USER \
...
# Set CSD_HOSTNAME to your vpnhost and export it to ENV
if [[ -z ${CSD_HOSTNAME} ]]
then
...
# creating dirs
for dir in $HOSTSCAN_DIR $LIB_DIR $BIN_DIR $TMP_DIR ; do
    if [[ ! -f $dir ]]
    then
        mkdir -p $dir
    fi
...
then
    ...
    for i in ${FILES[@]} ; do
        ...
        then
            ...
            gzip -dk $FILE_GZ  # preserve gz file so that it won't get downloaded again next time
        fi
    ...
    for i in ${BINS[@]} ; do
        ...
        [ -f "$BIN_DIR/$i" ] && chmod u+x $BIN_DIR/$i
    done
